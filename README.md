...
> 以上から、作者は**規約違反や法的な問題はないと考えています。**
> そのうえで、過剰なアクセスにならないよう制限を設け、節度をもって使う限りは、
> 公開して差し支えないと判断しました。空き状況を探しやすくすることで、
> わずかではあっても**献血の利便性を高められる**と考えています。
>
...
> **この制限を外して利用しないでください。**
> 本アプリはソースコードを公開しているため、自分でビルドすれば制限を外すことも技術的には可能ですが、
> **そうして生じたいかなる問題についても作者は責任を負いません**（[改変して使う場合](#改変して使う場合)）。
>
...
> 次に起動したときから検索画面が出なくなります（[利用停止の仕組み](#利用停止の仕組み)）。
...
1. 初回起動時に**生年月日と性別**を登録します（[なぜ必要か](#生年月日と性別について)）
2. 検索方向を選びます
...
初回起動時に生年月日と性別の登録をお願いしています。あとから変更もできます。

**なぜ必要か**：献血予約サイトの空き状況ページは、生年月日と性別を条件として受け取る作りになっています。
以前のバージョンは固定のダミー値（2006年1月1日生まれの男性）を送っていましたが、
**架空の値を送るのは不誠実である**ため、ご自身の値を登録していただく形に改めました。

**この情報の扱い**

- 献血予約サイトへ**空き状況を照会するときにのみ**使用します
- **作者を含む第三者へ送信されることはありません。**
  アプリが外部へ送るのは、この照会と、起動時の設定の読み取りだけです
- 保存先はお使いのPC内の `%APPDATA%\KenketsuAvailability\settings.json` だけです

## 動作するバージョンについて

献血予約サイトのページ構成が変わるなどして正しく動かなくなった版は、
作者の設定で使用不可にすることがあります。該当した場合、アプリは検索画面を出さず
最新版のダウンロードを案内します。お手数ですが更新してください。
...
本アプリは [MIT License](LICENSE) でソースコードを公開しています。ライセンスの定めるとおり、
改変は自由です。そのため、**自分でビルドすればレートリミットを外したり、
利用停止の仕組みを取り除いたりすることも技術的には可能です。**

そのうえで、次の点を明記しておきます。

- 配布している exe を書き換えた場合、ソースコードを改変してビルドした場合、
  その他どのような方法であれ、**改変された状態での利用によって生じたいかなる問題についても、
  作者は一切の責任を負いません。**
- とりわけ、**日本赤十字社から警告・利用停止の要請・法的措置その他の対応を受けた場合も、
  作者は関知しません。** すべて改変して利用した方ご自身の責任で対応していただくことになります。
- 改変版は本アプリとは別物です。不具合や損害について作者へお問い合わせいただいても対応できません。

## 利用停止の仕組み

日本赤十字社からご要請をいただいた場合などに、
**すでにダウンロード済みのアプリも検索できない状態にできる**ようにしています。
...
- ソースコードを改変した版の利用についても同様です（[改変して使う場合](#改変して使う場合)）。
